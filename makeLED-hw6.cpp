...
int main(int argc, char* argv[]){
   if(argc!=3){
	cout << "First arg is either on, off, flash, status, or blink." << endl;
	cout << "Second arg is optional, takes an integer;" << endl;
   }

   string cmd(argv[1]);
   string cmd2(argv[2]);
   int n = atoi((cmd2.c_str()));
   ...
   /////////////////////////////////////////////////////////////////// HW MODIFICATION
	...
	for (int i = 0; i < n; i++){
		writeLED("/brightness", "1"); // turn on
		sleep(1);		// sleep one second
		writeLED("/brightness", "0"); // turn off
		sleep(1);		// sleep one second
		cout << "USR3 has blinked: " << i+1 << " times" << endl;
	} // repeat n times
   }
   else{
